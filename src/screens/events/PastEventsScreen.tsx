...
import {fetchPastEventsThunk} from '../../redux/thunks/event/fetchPastEventsThunk';
...
const PastEventsScreen = ({
  searchQuery,
  onEventSelect,
}: PastEventsScreenProps) => {
  const dispatch = useDispatch();
  const {pastEvents, isLoading, error} = useSelector(
  ...
  const renderEvent = ({item, index}) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.eventCard, itemWidth, styles.pastEventCard]}
...
const styles = StyleSheet.create({
  ...
  pastEventCard: {
    opacity: 0.8, // Style spécifique pour les événements passés
  },
  eventContent: {
  ...
  eventDate: {
    fontSize: scale(14),
    color: colors.grey, // Couleur différente pour les événements passés
  },
  errorText: {
    color: colors.red,
    fontSize: scale(16),
