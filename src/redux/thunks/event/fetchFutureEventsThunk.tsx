...
export const fetchFutureEventsThunk = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      ...
      const response = await fetchEventList(userId, 'future');
      
      if (!response.status || !response.event_details) {
        return thunkAPI.rejectWithValue(response.message || 'Failed to fetch future events');
      }
      ...
      return thunkAPI.rejectWithValue(error.message || 'Error fetching future events');
    }
  }
);
